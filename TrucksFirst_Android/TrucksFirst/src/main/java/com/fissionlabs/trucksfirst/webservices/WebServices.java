...
//                headers.put("Content-Type", "application/x-www-form-urlencoded");
    ...
    public void getSOSTrucksList(final Context context, final ResultReceiver resultReceiver) {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, URL_SOS_TRUCKS_LIST + "?hubName=" + TFUtils.getStringFromSP(context, HUB_NAME),
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        if (LogConfig.D) {
                            Log.d(TAG, "================================ SOS details ==========================");
                            Log.d(TAG, response);
                            Log.d(TAG, "================================ SOS details end ======================");
                        }
                        Bundle bundle = new Bundle();
                        bundle.putString("response", response);
